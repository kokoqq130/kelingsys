...
- 原始报告、出院记录和处方图片是原始证据，不因后续结论改变而覆盖。
- 住院专用文档保存当次事件；后续补充资料时注明来源和更新时间，不把现在的状态倒写成当时状态。
- 主文档“目前治疗方案”是当前用药唯一权威入口，并保留明确截至日期；住院文档里的药物内容视为历史调药事件。
- 新旧信息冲突时，保留原时间轴记录，在新日期追加变化；无法确认时写“待核对”，不要自行选择一个值覆盖另一个值。
- PDF、SQLite 索引、前端页面和静态 JSON 都是派生产物，不得反向覆盖 markdown 或原始报告。
- 住院文档可以根据实际情况自然分节，但优先保留清楚的字段标签，例如“入院时间”“出院时间”“入院原因”“发作经过”“住院处理”“主要症状”“发作后用药调整”。
- 完成资料更新后，运行项目根目录的 `scripts/Check-ProjectConsistency.ps1`，检查链接、索引和过期派生产物提示。
...
5. 确认当前用药只在主文档“目前治疗方案”中作为当前结论，住院文档只记录当次调药。
6. 运行 `scripts/Check-ProjectConsistency.ps1`，处理其中的错误，并阅读过期 PDF 等警告。
