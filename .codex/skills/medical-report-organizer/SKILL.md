...
资料整理模式下，OCR 统一通过本 skill 里的 `Invoke-MedicalOcr.ps1` 调用 `app/backend/.venv` 内的 PaddleOCR。后续如果 OCR 方案再切换，这里需要和工作区根目录的 `AGENTS.md` 一起同步更新。
...
- 优先按“住院周期”整理，而不是只写单个入院日期。
- 至少记录入院时间、可选的出院时间、当前状态、入院原因、触发事件、主要症状、即时处理。
- 如果已经出院但暂时没有出院小结，要明确写“出院信息待补”或“出院小结待补”。
- 同一次住院的出院小结、出院记录、报告索引，都尽量放在同一个住院文件夹下。
